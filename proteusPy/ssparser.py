...
def extract_ssbonds_and_atoms(input_pdb_file, verbose=False, dbg=False) -> tuple:
    ...
    for line in lines:
        if line.startswith("SSBOND"):
            ...
            # pdb_dict["secondary_structure"]["helices"].append(helix_info)

        elif line.startswith("SHEET"):
            sheet_info = parse_sheet_record(line)
            sheets.append(sheet_info)
            # pdb_dict["secondary_structure"]["sheets"].append(sheet_info)

        elif line.startswith("TURN"):
            turn_info = parse_turn_record(line)
            turns.append(turn_info)
            # pdb_dict["secondary_structure"]["turns"].append(turn_info)
