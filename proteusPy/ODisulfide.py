...
def Extract_Disulfides(
    ...
    :param prune:          Move non parseable disulfides, defaults to True
    """

    def name_to_id(fname: str) -> str:
        """
        Return the PDB ID from the filename.
    ...
    # If numb is passed then only do the last numb entries.
...
def Extract_Disulfide(
    ...
    """
    Read the PDB file specified by `pdbid` and returns a `DisulfideList` object.

    This function parses the specified PDB file to identify disulfide bonds and encapsulates the resulting `Disulfide` objects within a `DisulfideList` object.

    Parameters:
    - `pdbid` (str): The ID of the PDB file to be read.
    - `verbose` (bool): If True, enables the output of additional messages during processing.
    - `quiet` (bool): If True, suppresses warnings from `DisulfideConstruction`.
    - `extra` (bool): If True, enables the pruning of duplicate disulfide bonds.

    Returns:
    - `DisulfideList`: An object containing the identified `Disulfide` bonds.
    ...
    if len(sslist) == 0:
        if verbose:
            print(f"--> No disulfides parsed for: {pdbid}")
    else:
        if extra:
            sslist = remove_duplicate_ss(_sslist)
