...
speech signals---rarely uses all of its ambient dimensions. Handwritten digits
live on a manifold of intrinsic dimension roughly 13 inside 64-dimensional pixel
space. CIFAR-10 natural images sit on a manifold of intrinsic dimension roughly
29 inside 3{,}072-dimensional space. More than 99\% of the ambient dimensions
are noise.
...
\begin{tikzpicture}[
  ...
  block/.style={
    ...
    minimum width=7.4cm, minimum height=1.0cm,
...
%% ── Layer boxes (bottom → top) ────────────────────────────────────────────
\node[block, fill=blue!12]   (L1) at (0,0)
  {\textbf{Layer 1: TurtleND}\\[1pt]
   Position $\mathbf{p}\in\mathbb{R}^N$, orthonormal frame $\mathbf{F}$,
   Givens rotations};

\node[block, fill=teal!12]   (L2) at (0,2.0)
  {\textbf{Layer 2: ManifoldWalker}\\[1pt]
   KNN $\to$ local PCA $\to$ project gradient onto tangent plane $\to$ step};

\node[block, fill=orange!18] (L3) at (0,4.0)
  {\textbf{Layer 3: ManifoldModel}\\[1pt]
   KNN graph + stored PCA geometry $\to$ manifold-weighted nearest-neighbour vote};

\node[block, fill=red!12]    (L4) at (0,6.0)
  {\textbf{Layer 3a: ManifoldObserver}\\[1pt]
   Extend $N\!\times\!N$ frame by QR $\to$ $(N\!+\!1)$-dim turtle
   sees curvature, topology, class boundaries};
...
%% ── Side annotations ──────────────────────────────────────────────────────
\node[lbl, left=0.55cm of L1] (aL1) {navigation\\primitive};
\node[lbl, left=0.55cm of L2] (aL2) {Riemannian-\\approx.\\ optimizer};
\node[lbl, left=0.55cm of L3] (aL3) {zero-param\\classifier};
\node[lbl, left=0.55cm of L4] (aL4) {extrinsic\\observer};

%% ── Right-side: "gains" annotations ──────────────────────────────────────
\node[lbl, right=0.45cm of L1, text=blue!60!black]
  {orientation \&\\local frame};
\node[lbl, right=0.45cm of L2, text=teal!60!black]
  {stays on the\\manifold};
\node[lbl, right=0.45cm of L3, text=orange!70!black]
  {geometry\\IS the model};
\node[lbl, right=0.45cm of L4, text=red!60!black]
  {gains the\\normal: sees\\the surface};

%% ── Bottom label ──────────────────────────────────────────────────────────
\node[font=\footnotesize, below=0.35cm of L1]
  {$N$-dimensional input data (tabular, image PCA, molecular angles, embeddings, \ldots)};

\end{tikzpicture}
\caption{The WaveRider stack.  Each layer builds on the layer below: TurtleND
provides the navigation primitive; ManifoldWalker uses it for gradient descent
constrained to the data manifold; ManifoldModel stores the explored geometry as
a zero-parameter classifier; ManifoldObserver extends the frame by one QR step
to gain the manifold normal and an extrinsic view of the surface.}
\label{fig:stack}
\end{figure}
%% ─────────────────────────────────────────────────────────────────────────────
