...
from generate_class_disulfides import (
    generate_disulfides_for_class_from_csv,
    generate_disulfides_for_selected_classes
)


def compare_energy_distributions(class_disulfides: Dict[str, DisulfideList], class_names: Dict[str, str]) -> None:
    """
    ...
    for i, (class_id, disulfide_list) in enumerate(class_disulfides.items()):
        ...
        plt.hist(energies, bins=20, alpha=0.3, color=color, label=f'Class {class_id} ({class_names[class_id]})')
        
        # Plot vertical lines for mean and min energy
        plt.axvline(mean_energy, color=color, linestyle='dashed', linewidth=2, 
                   label=f'Mean {class_id}: {mean_energy:.2f}')
        plt.axvline(min_energy, color=color, linestyle='dotted', linewidth=2, 
...
def compare_dihedral_distributions(class_disulfides: Dict[str, DisulfideList], class_names: Dict[str, str]) -> None:
    """
    ...
    for i, (class_id, disulfide_list) in enumerate(class_disulfides.items()):
        ...
        axs[0].hist(chi1_values, bins=20, alpha=0.3, color=color, 
    ...
    print("Saved dihedral distribution comparison plot to class_dihedral_comparison.png")
    
    # Show the plot (comment out if running in a non-interactive environment)
    # plt.show()


def compare_minimum_energy_disulfides(class_disulfides: Dict[str, DisulfideList], class_names: Dict[str, str]) -> Dict[str, Disulfide]:
    """
    ...
    print("-" * 50)
    print(f"{'Class ID':<10} {'Class Name':<15} {'Energy (kcal/mol)':<20} {'Cα Distance (Å)':<15}")
    print("-" * 50)
    ...
    for class_id, disulfide_list in class_disulfides.items():
        ...
        print(f"{class_id:<10} {class_names[class_id]:<15} {min_energy_disulfide.energy:<20.2f} {min_energy_disulfide.ca_distance:<15.2f}")
    ...
    for i, bar in enumerate(bars):
        plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.1, 
                class_names[class_ids[i]], ha='center', va='bottom', rotation=45)
...
def compare_average_conformations(class_disulfides: Dict[str, DisulfideList], class_names: Dict[str, str]) -> Dict[str, Disulfide]:
    """
    ...
    print("-" * 80)
    print(f"{'Class ID':<10} {'Class Name':<15} {'Chi1':<10} {'Chi2':<10} {'Chi3':<10} {'Chi4':<10} {'Chi5':<10}")
    print("-" * 80)
    ...
    for class_id, disulfide_list in class_disulfides.items():
        ...
        avg_disulfide = Disulfide(name=f"{class_id}_{class_names[class_id]}_avg", torsions=avg_conformation)
        avg_conformation_disulfides[class_id] = avg_disulfide
        
        # Print information about the average conformation
        print(f"{class_id:<10} {class_names[class_id]:<15} {avg_conformation[0]:<10.2f} {avg_conformation[1]:<10.2f} {avg_conformation[2]:<10.2f} {avg_conformation[3]:<10.2f} {avg_conformation[4]:<10.2f}")
    ...
    for i, (class_id, avg_disulfide) in enumerate(avg_conformation_disulfides.items()):
        ...
        ax.plot(angles, values, color=color, linewidth=2, marker=marker, label=f'{class_id} ({class_names[class_id]})')
    ...
    print("Saved average conformation comparison plot to class_avg_conformation_comparison.png")
...
def main():
    ...
    # Generate disulfides for the selected classes
    ...
    print("-" * 50)
    print(f"{'Class ID':<10} {'Class Name':<15} {'Count':<10} {'Avg Energy':<15} {'Avg Cα Dist':<15}")
    print("-" * 50)
    
    for class_id, disulfide_list in class_disulfides.items():
        print(f"{class_id:<10} {class_names[class_id]:<15} {len(disulfide_list):<10} {disulfide_list.average_energy:<15.2f} {disulfide_list.average_ca_distance:<15.2f}")
