...
ingestion stack (``pepys_diverse_embeddings.json``, 2356 sentences from the
...
4. ManifoldWalker flight: pick the two sentences most distant in cosine space,
...
-----
  ...
  # quick test (500-sentence temporal subset)
...
def ingest_diary(
    ...
    Produces one text string per sentence: ``semantic_category | content``.
    Each diary entry becomes 1–3 sentences at sentence boundaries.
    ...
    :param max_chunks_per_entry: Max sentences per entry (0 = unlimited).
    ...
    console.print(
        f"  Produced {len(texts)} sentences from {len(entries)} entries  "
...
def main() -> None:  # noqa: C901
    ...
    if args.n and args.n < len(texts):
        ...
        console.print(
            f"  Temporally sampled {len(texts)} sentences  "
            f"({timestamps[0].date()} → {timestamps[-1].date()})"
        )
    ...
    if retrieval_pairs:
        ...
        console.print(
            ...
            f"avg {np.mean([len(r) for r in rel_lists]):.0f} relevant sentences/query"
        )
        try:
